...
def Point_addition(eng):
    ...
    if (resource_check != 1):
        Round_constant_XOR(eng, x1, 0x40, n)
        Round_constant_XOR(eng, y1, 0x04, n)
    ...
    anc0, ancilla = Inverison_Itoh_Tsujii_based(eng, x1, ancilla)  # need reverse
    ...
    anc3, ancilla = Inverison_Itoh_Tsujii_based(eng, x1, ancilla)  # need reverse

    count = 0
    anc4, count, ancilla = recursive_karatsuba(eng, y1, anc3, n, count, ancilla)  # need reverse
    anc4 = Reduction(eng, anc4)

    CNOT127(eng, anc4, lambda_0)

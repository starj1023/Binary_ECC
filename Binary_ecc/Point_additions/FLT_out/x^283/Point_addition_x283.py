...
def Point_addition(eng):
    ...
    Round_constant_XOR(eng, x1x2, x2, n)
    CNOT283(eng, x1x2, result2)  # (x2+x3)lambda + y2 + x3
...
def Inverison_Itoh_Tsujii_based(eng, a) :
    ...
    H_2_26 = Squaring_2_26(eng, H, a2, n)  # reverse
...
def Reduction(eng, result): # x^283 + x^12 + x^7 + x^5 + 1
    n = 283
    for i in range(n - 1):
        if (i < n):
...
def recursive_karatsuba_omit_reverse(eng, a, b, n, count, ancilla):
